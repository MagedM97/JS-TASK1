...
const rendering =()=>{
    ...
    product.forEach((p,i)=>{document.getElementById("products").innerHTML+=add_row(p,i)})
...
if (productName !=""&&productPrice !=""&&productNumber !=""&& !Number.isNaN(productPrice)&& productPrice > 0 &&Number.isInteger(productNumber) && productNumber > 0 && !currentProducts.includes(productName))
{product.push({productName: productName, productPrice:productPrice,productNumber:productNumber});
localStorage.setItem("products", JSON.stringify(product));
rendering();}
else 
{alert ("check Product name , Price & Quantity again please or Product already exsit you can change quantity");
return ("")}
};
const add_row=(p,i)=>{
...
const subtotal=()=>{
  let st=  product.map((p)=>p.productPrice*p.productNumber).reduce((a,t)=>a+=t,0);
  return (st+"$");
}
const shipping=()=>{
